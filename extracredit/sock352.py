...
class Socket:
    ...
    def __init__(self):
        ...
        # create dictionaries of the keys 
        self.privateKeys = {}
        self.privateKeysHex = {}
        self.publicKeys = {}
        self.publicKeysHex = {} 
    ...
    def sendto(self, buffer):
        self.sequence += 1
        encrypted_buffer = self.box.encrypt(buffer, self.nonce)
        packet = Packet(cntl = DATA,
            seq = self.sequence,
            size = len(encrypted_buffer),
        ...
        while True:
            ...
            if packet.cntl == DATA:
                if packet.seq == self.receive_sequence:
                    ...
                    decrypted_data = self.box.decrypt(packet.data)
    ...
    def setup_encryption(self):
        key_key = (self.send_address[0], str(self.send_address[1]))
        public_key = None
        #Need to find public key that matches address and port
        #check for port specific key first
        if key_key in self.publicKeys:
            public_key = self.publicKeys[key_key]
        else:
            #find global key
            public_key = self.publicKeys[(self.send_address[0], '*')]

        #gets the private key
        private_key = self.privateKeys.itervalues().next()
